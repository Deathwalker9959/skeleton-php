...
class Router
{
    ...
    private function handleMiddleware($controllerMiddleware, $group, $models)
    {
        ...
        if (isset($group['middleware'])) {
            ...
            foreach ($group['middleware'] as $middleware) {
                ...
                $qualifiedClass = "Skeleton\\Middleware\\" . ucFirst($middleware);
    ...
    {
        $response = response()->status(404)->view("error.404");
        $response->send();
        return $response;
    }
    ...
    private function transformRouteKeys($key, $prefix = null)
    {
        return "/{$prefix}{$key}";
    }
    ...
    public function matchUrlToRoute($url, $method, $route)
    {
